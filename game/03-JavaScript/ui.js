...
function getLargestSexStatModifierCssClasses(input, requiredLevel = 0) {
	...
	if (
		...
		// Sets the animation based on how large the modifier is. Without this level css, only the glow effect will be present on the text.
...
$(document).on(":passagedisplay", function () {
	distributePassageSexStatModifierCss();
	distributeCombatSexStatModiferCss();
	offsetSexStatModifierAnimations();
});

// Applies the drunk/jitter css effects to the first passage link behind any sexstat-widgets with the drunk/jitter effect.
function distributePassageSexStatModifierCss() {
	const modifiedElements = document.querySelectorAll(".drunk-text.sexstat-widget, .jitter-text.sexstat-widget");

	for (const element of modifiedElements) { 
		const modifierType = element.classList.contains("drunk-text") ? "drunk" : "jitter";
		const modifierLevel = element.className.match(new RegExp(`${modifierType + "-"}\\d`));

		// Add the classes to the previous <a data-passage> element
		$(element).prevAll("a[data-passage]").first().addClass(`${modifierType}-text`).addClass(modifierLevel ?? "");
	}
}

// Adds the drunk/jitter effect to all children of <label>s with a child that has drunk-text or jitter-text. Done to apply the effect to all elements in combat options.
function distributeCombatSexStatModiferCss() {
	const drunkModifiedLabels = document.querySelectorAll("label:has(.drunk-text.sexstat-widget)");
	for (const label of drunkModifiedLabels) {
		const drunkTextElement = label.querySelector(".drunk-text.sexstat-widget");
		const drunkLevelClass = drunkTextElement.className.match(/(drunk)-\d+/)?.[0];

		for (const combatOptionElement of label.children) {
			if (combatOptionElement.type !== "radio") {
				combatOptionElement.classList.add("drunk-text", drunkLevelClass);
			}
		}
	}

	const jitterModifiedLabels = document.querySelectorAll("label:has(.jitter-text.sexstat-widget)");
	for (const label of jitterModifiedLabels) {
		const jitterTextElement = label.querySelector(".jitter-text.sexstat-widget");
		const jitterLevelClass = jitterTextElement.className.match(/(jitter)-\d+/)?.[0];

		for (const combatOptionElement of label.children) {
			if (combatOptionElement.type !== "radio") {
				combatOptionElement.classList.add("jitter-text", jitterLevelClass);
			}
		}
	}
}

// Add a random offset to the animations of elements with drunk-text or jitter-text to make them look better.
function offsetSexStatModifierAnimations() {
	const modifiedElementList = document.querySelectorAll(".drunk-text, .jitter-text");

	for (const modifiedElement of modifiedElementList) {
		const currentAnimationTime = parseFloat($(modifiedElement).css("animation-duration"));
		
		// Skip if there isn't an animation
		if (isNaN(currentAnimationTime)) continue;

		modifiedElement.style.animationDuration = `${currentAnimationTime * (1 + (Math.random() * .3 - .15))}s`;
		modifiedElement.style.animationDelay = `${Math.random() * -5}s`;
	}
}

// Used to display the drunk text, with with animations if enabled, otherwise just the glow effect.
// Used for displaying the drunk text effect in passages where the text may be very long, and we don't want a huge wobble on it.
...
// Used for displaying the jitter in passages. For consistency with basicDrunkCss() usage.
