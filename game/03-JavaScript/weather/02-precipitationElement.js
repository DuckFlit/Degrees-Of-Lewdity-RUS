...
class SkyCanvasPrecipitation extends SkyCanvasAnimationEffects {
	...
	addPrecipitation(cloudPositions) {
		...
		if (spriteFrames.length > 0) {
			cloudPositions.forEach(({ x, y, width, height }) => {
				const spriteWidth = spriteFrames[0].width;
				const spriteHeight = spriteFrames[0].height;
				const startY = y + height + settings.verticalOffset;
				const endY = this.canvasElement.height;

				const spriteCountX = Math.max(1, Math.floor(width / (spriteWidth + settings.spriteOverlap)));
				...
				for (let i = 0; i < spriteCountX; i++) {
					for (let j = 0; j < spriteCountY; j++) {
						...
						const effect = new this.Precipitation(this, spriteFrames, effectX, effectY, spriteWidth, spriteHeight, settings.fps);
	...
	redrawEffects() {
		...
		const alpha = this.interpolate(0.15, settings.opacity, factor);
