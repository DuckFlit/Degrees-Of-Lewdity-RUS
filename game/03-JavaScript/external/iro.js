...
function redrawImg(tmp_blend, tmp_brightness, eye_colors, eye_colors2) {
  ...
  redrawImg.id = setTimeout(() => {
    ...
    setup.colours.eyes[eye_colors].canvasfilter.blend = tmp_blend;
    if (tmp_brightness != undefined)
      setup.colours.eyes[eye_colors].canvasfilter.brightness = tmp_brightness;
    if (eye_colors2 != undefined){
      V.custom_eyecolours[eye_colors2].canvasfilter.blend = tmp_blend;
      if (tmp_brightness != undefined)
        V.custom_eyecolours[eye_colors2].canvasfilter.brightness = tmp_brightness;
    }
  }, 50);
}

window.initCustomLenses = function (){
	let found, i, i2;
...
var colorPicker = new iro.ColorPicker('#picker', {
    colors: [
      'rgb(72, 250, 235)', // light blue
      // 'rgb(225, 188, 55)', // light orange
    ],
    layout: [
      { 
        ...
        if (setup.colours.eyes[eye_colors].variable == ((V.makeup.eyelenses == 0) ? V.eyecolour : V.makeup.eyelenses)){
          ...
          for (eye_colors2 in V.custom_eyecolours){
            if (V.custom_eyecolours.length != 0 && V.custom_eyecolours[eye_colors2].variable == V.makeup.eyelenses){
              ...
              V.custom_eyecolours[eye_colors2].canvasfilter.brightness = color.hsl["l"] <= 0 ? 0 : color.hsl["l"] / 500
              break;
            }
            eye_colors2 = undefined
          }
          redrawImg(tmp_blend, tmp_brightness, eye_colors, eye_colors2);
          break;
        }
      }
      if (check_condition == 0){
        Errors.report("No match found between your eyelenses color and the ones in setup.colours.eyes\neye_color : " + V.eyecolour + " eyelens : " + V.makeup.eyelenses + "\nPlease report this issue.")
      }
    }
  });
  /* adding button */
